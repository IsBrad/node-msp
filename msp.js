...
var utils = {
  joinUints : function (uints) {
    var out = uints[uints.length - 1];
    for (var index = uints.length - 2; index >= 0; index--) {
      out = (out << 8) + uints[index];
    }
    return out;
  },
  splitUint : function (uint, amount) {
...
var messageCodes = {
  ...
  105 : {
    ...
    handler : function (parts) {
      ...
      for (var index = 9; index < parts.length; index+=2) {
  ...
  200 : {
    ...
    builder : function (options, callback) {
      var confine = function (number, rest, max, min) {
  ...
  203 : undefined,
  204 : undefined /* leaving blank as different for cleanflight. will work on at later date{
  ...
  212 : undefined /* undefined for now {
    name : 'MSP_SET_SERVO_CONF',
    constructor : function (options, callback) {
...
var mspReader = function () {
  ...
  this.messageComplete = function () {
    ...
    var messageInfo = messageCodes[message.id];
...
module.exports = {
  ...
  send : function (code, optionsInput) {
    ...
    if (messageCodes[code] == undefined) {
      ...
      var parts = messageCodes[code].builder(options);
      ...
      var xor = word[3] ^ word[4];

      //Payload
      for (var index = 0; index < parts.length; index++) {
          word[index + 5] = parts[index];
          xor = xor ^ word[index + 5];
      }
